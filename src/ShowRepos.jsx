    ...
    return(
        <div className="flex gap-6 m-3 border-4 border-blue-500 p-2 pt-1 rounded-xl">
            <div className="w-1/2">
                ...
                <form className='flex rounded-lg border-2 border-gray-600 overflow-hidden focus-within:border-purple-600'
                    onSubmit={onClickSearchUsername} 
                    >
                    <input 
                        className=' w-full h-10 p-2 text-xl font-medium focus-within:outline-none ' 
                        type='text' placeholder='GitHub Username' name="username" autoComplete="off" /> {/*className='searchRepoInput'  */}
                    <button 
                        type='submit' 
            ...
            <div className="w-1/2 relative"
                ...
                <div 
                    className="rounded-lg border-2 border-gray-600 overflow-hidden focus-within:border-purple-600">
                    <input 
                ...
                {filtered.length!==0
                    ?
                    <ul
                        className="reposButtonsDiv absolute left-0 right-0 mt-2 z-50 rounded-xl overflow-hidden border-2 border-purple-400"
                    >
                        {filtered.map((repo,index)=>{
                            return (
                                <li 
                                key={repo.name} 
                                className={(index===highLightedIndex?' bg-stone-200 text-blue-700 ':'')+' selectRepoButton text-center ' }
                                >
                                    {repo.name}
                                </li>)
                        })}
                        <li className={(filtered.length<repos.length?'block':'hidden')+' text-center'}>{filtered.length<repos.length?repos.length-filtered.length:''} More Repositories</li>
                        <li className="text-center hover:bg-red-200" onClick={()=>setFiltered([])}>Cancel!</li>
                    </ul>
                    :''
                }
            </div>
